...
// ============================================================  
  private suspend fun processBatchWithFallback(
   ...
   // فحص سلامة سجلات الجدول
// ============================================================

private fun needsTableAlignmentRepair(
...
// ============================================================
// إصلاح محاذاة صفوف الجدول
// ============================================================

private suspend fun repairTableAlignment(
    image: PreparedImage,
    fields: List<ExtractionField>
): List<ExtractionResult> {
        ...
        fields.joinToString("\n") { field ->
            ...
            val description =
                field.description
                    .trim()
                    .ifBlank {
                        "اقرأ القيمة الظاهرة في هذا العمود."
                    }
...
- اسم الحقل: "${field.name}"
  الوصف: $description
     ...
     * مهم جدًا:
     *
     * لا نرسل النتيجة السابقة إلى النموذج.
     *
     * لأن النتيجة السابقة تحتوي على الانزياح نفسه،
     * وإرسالها قد يجعل النموذج يعيد نفس الخطأ.
     *
     * الصورة هي المصدر الوحيد للحقيقة.
     */
    val repairPrompt = """
أنت محرك OCR متخصص في قراءة الجداول واستخراج السجلات.

لديك صورة واحدة تحتوي على جدول أو قائمة تحتوي على عدة سجلات.

أعد قراءة الصورة من البداية.

==================================================
المهمة الأساسية
==================================================

استخرج كل صف مرئي في الجدول كسجل مستقل.

كل صف أفقي في الصورة يمثل سجلًا واحدًا.

لا تفصل الأعمدة ثم تعيد تركيبها لاحقًا.

بدل ذلك:

اقرأ الصف الأول كاملًا كوحدة واحدة.

ثم الصف الثاني كاملًا كوحدة واحدة.

ثم الصف الثالث كاملًا كوحدة واحدة.

وهكذا حتى آخر صف ظاهر.

==================================================
قاعدة محاذاة الصفوف
==================================================

القيمة الموجودة في العمود الأول من الصف يجب أن تبقى
مرتبطة بالقيم الموجودة في نفس الصف.

لا تنقل اسمًا إلى الصف التالي.

لا تنقل رقمًا إلى الصف السابق.

لا تجعل بيانات صف مرتبطة باسم صف آخر.

لا تستخدم ترتيب الأسماء من الذاكرة.

لا تستنتج العلاقة بين الأعمدة بالاعتماد على المعنى.

العلاقة الوحيدة الصحيحة هي:

نفس الصف في الصورة = سجل واحد.

==================================================
عدد السجلات
==================================================

احسب عدد الصفوف الفعلية الظاهرة في الصورة بنفسك.

لا تفترض أن عدد السجلات يجب أن يكون رقمًا معينًا.

لا تضف سجلًا غير موجود.

لا تكرر سجلًا.

لا تنشئ سجلًا فارغًا.

إذا كانت الصورة تحتوي على 29 صفًا:
أعد 29 سجلًا فقط.

إذا كانت تحتوي على 30 صفًا:
أعد 30 سجلًا فقط.

إذا كانت تحتوي على N صفوف:
أعد N سجلات بالضبط.

لا تحاول مطابقة عدد ثابت مسبقًا.

==================================================
 الصفوف الفارغة
==================================================

لا تعتبر صفًا فارغًا أو مسافة بين الصفوف سجلًا.

لا تنشئ سجلًا جديدًا بسبب فصل بصري بين صفين.

==================================================
الصف الأخير
==================================================

تأكد من قراءة آخر صف ظاهر في الصورة.

لا تتوقف قبل نهاية الجدول.

==================================================
الأسماء
==================================================

إذا كان أحد الحقول اسم شخص:

اقرأ الاسم من نفس الصف الذي توجد فيه بقية بيانات الشخص.

لا تنقل الاسم إلى صف آخر.

لا تكرر اسمًا سبق قراءته.
...
اقرأ الرقم الموجود في نفس الصف.

لا تنقل الرقم من صف إلى آخر.
...
لا تعتمد على أي استخراج سابق.

لا تفترض أن النتيجة السابقة صحيحة.

أعد قراءة الجدول بصريًا من البداية.
...
كل عنصر داخل results يمثل صفًا واحدًا فقط.

لا تضع صفين داخل عنصر واحد.

لا تقسم صفًا واحدًا إلى عنصرين.
...
ابدأ القراءة من أول صف فعلي في الصورة
وانتهِ عند آخر صف فعلي في الصورة.
""".trimIndent()
    ...
    return try {
        ...
        /*
         * يجب أن تكون النتيجة متعددة السجلات.
         */
        if (parsed.size <= 1) {
            emptyList()
        } else {
         ...
         * الإصلاح اختياري.
         *
         * إذا فشل طلب الإصلاح لا نفشل عملية
...
// ============================================================
// إصلاح الجدول عند الحاجة فقط
// ============================================================

private suspend fun repairTableIfNeeded(
    image: PreparedImage,
    fields: List<ExtractionField>,
     ...
     * طلب إصلاح واحد فقط.
     *
     * لا نرسل طلبًا لكل صف.
     *
     * هذا يحافظ على سرعة Mistral في الحالات الطبيعية
     * ويجعل الإصلاح محدودًا بحالة الاشتباه فقط.
     */
    val repairedResults =
        repairTableAlignment(
            image = image,
            fields = fields
        )

    /*
     * إذا نجح الإصلاح وأعاد سجلات فعلية،
     * نستخدم النتيجة الجديدة.
     */
    if (repairedResults.size > 1) {
        return repairedResults
    }

    /*
     * إذا فشل الإصلاح:
     * نحافظ على نتيجة Pass 1 بدل إظهار خطأ للمستخدم.
     */
    return initialResults
}     
